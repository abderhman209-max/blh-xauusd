  ...
  if (path === "/" || path === "/index.html") {
    ...
    body = body
      ...
      .replace(
        ...
        `s+=\`<line x1="0" y1="\${H-bottom}" x2="\${pw}" y2="\${H-bottom}" stroke="#2b2e36"/>\`;const labelEvery=Math.max(1,Math.ceil(64/Math.max(1,step)));for(let j=0;j<count;j++){const i=start+j;if(j%labelEvery!==0&&j!==count-1)continue;const date=new Date(sourceBars[i].time),previous=i>0?new Date(sourceBars[i-1].time):null,newDay=!previous||date.getUTCDate()!==previous.getUTCDate()||date.getUTCMonth()!==previous.getUTCMonth(),tx=j===0?4:j===count-1?pw-4:x(i),anchor=j===0?"start":j===count-1?"end":"middle",time=String(date.getUTCHours()).padStart(2,'0')+':'+String(date.getUTCMinutes()).padStart(2,'0');if(newDay){const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],dateText=date.getUTCDate()+' '+months[date.getUTCMonth()]+' \\''+String(date.getUTCFullYear()).slice(-2),boxWidth=74,boxX=Math.max(2,Math.min(pw-boxWidth,tx-boxWidth/2));s+=\`<line x1="\${tx}" y1="\${top}" x2="\${tx}" y2="\${H-bottom}" stroke="#343842" stroke-dasharray="2 4"/><rect x="\${boxX}" y="\${H-bottom+3}" width="\${boxWidth}" height="23" rx="5" fill="#292c33" stroke="#414550"/><text x="\${boxX+boxWidth/2}" y="\${H-8}" text-anchor="middle" fill="#f1f3f5" font-size="11" font-weight="600">\${dateText}</text>\`}else{s+=\`<text x="\${tx}" y="\${H-8}" text-anchor="\${anchor}" fill="#aeb2ba" font-size="11">\${time}</text>\`}}`,
      );
  }
